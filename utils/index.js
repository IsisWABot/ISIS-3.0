...
const processTime = (timestamp, now) => {
    return moment.duration((now - moment(timestamp * 1000))*-1).asMilliseconds()/1000
}

// is Url?
const Url = (url) => {
    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,512}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
}

const isUrl = (url) => {
    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/gi))
}
const isUrlGrupo = (url) => {
    return url.match(new RegExp(/(https:\/\/chat.whatsapp.com)/,'gi'))
}
